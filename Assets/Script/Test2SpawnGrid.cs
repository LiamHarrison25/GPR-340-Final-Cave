...
public class Test2SpawnGrid : MonoBehaviour
{
    ...
    private Dictionary<Vector3Int, ChunkObjectPool> chunkObjectPool;
    ...
    void Start()
    {
        ...
        chunkObjectPool = new Dictionary<Vector3Int, ChunkObjectPool>();
    ...
    private void GenerateChunks()
    {
        ...
        for(int x = -worldChunkRadius; x < worldChunkRadius; x += chunkSize)
        {
            for(int y = -worldChunkRadius; y < worldChunkRadius; y += chunkSize)
            {
                for (int z = -worldChunkRadius; z < worldChunkRadius; z += chunkSize)
                {
    ...
    void GenerateChunksObjectPool()
    {
        ...
        for (int i = 0; i < totalWorldRenderDistance; i++)
        {
            ...
            ChunkObjectPool newChunkObjectPool = new ChunkObjectPool()
            {
                position = chunkPosition,
                cells = new Dictionary<Vector3Int, GameObject>()
            };
            //add chunks to object pool
            chunkObjectPool.Add(chunkPosition, newChunkObjectPool);

            //create cells ----------------------------
            var newChunkCellsObjectPool = newChunkObjectPool.cells;
            ...
            newChunkObjectPool.theGameObject = chunkParent;
    ...
    void PositionObjectPool(bool checkForChunkChange)
    {
        ...
        Stack<ChunkObjectPool> removeChunkPositions = new Stack<ChunkObjectPool>();
        ...
        foreach (Vector3Int closeChunk in closestChunks)
        {
            ...
            if (!chunkObjectPool.ContainsKey(closeChunk))
            {
        ...
        foreach (var entry in chunkObjectPool)
        {
        ...
        {
            //grab a ChunkObjectPool from the remove list. steal it
            //set it to a different value

            ChunkObjectPool chunkObject = new ChunkObjectPool(); // Declare a variable for the output
            Vector3Int removedChunkKey = removeChunkPositions.Pop().position; // Get the key from the popped ChunkObjectPool

            //steal deleted chunks items
            chunkObject = chunkObjectPool[removedChunkKey];

            //correct it's position
            chunkObject.position = addThisPosition;
            chunkObject.theGameObject.transform.position = addThisPosition;

            //add new chunk with the stolen items, and set the required position
            chunkObjectPool.Add(addThisPosition, chunkObject);

            // Remove the old entry and add the new one
            chunkObjectPool.Remove(removedChunkKey);
        }
    }
    ...
    private HashSet<Vector3Int> ReturnClosestChunks()
    {
        ...
        int chunkWorldLength = worldChunkRadius * 2;
